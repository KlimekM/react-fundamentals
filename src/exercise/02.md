...
This exercise was interesting since I remember using React.createElement way
back when but have not touched it in some time and needed to reference the
documentation to see the parameters the the function accepts. I was surprised
that children can be passed either in props or as the 3rd argument and would
have guessed that it always gets passed in within the props object. The other
surprise was that I needed to add a ' ' between the two Hello World spans to get
a space to show though that should not be terribly surprising since a space is
usually inserted in the JSX when writing React code.
