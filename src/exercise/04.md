...
This exercise was insightful in that I learned that the first argument to
React.createElement can also be a function which makes sense considering that in
React every component can be a function that gets passed around. The other thing
that tripped me up and this may be from not using propTypes in a while but the
propTypes declaration had to be directly below the Message component for an
error to throw vs being at the bottom of the script tag.
